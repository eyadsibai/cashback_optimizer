...
def _build_optimization_problem(
    ...
    if lifestyle_card:
        plan_vars = LpVariable.dicts("PlanChoice", (p.name for p in lifestyle_card.plans), cat=LpBinary)
        activated_bonus_vars = LpVariable.dicts("ActivatedBonus", (p.name for p in lifestyle_card.plans), lowBound=0)

    # --- Objective Function ---
    all_cashback_components = []
    
    for card in cards:
        total_spend_on_card = lpSum(spend_vars[card.name, cat.key] for cat in categories.values())
        M = 1000000  # A large number

        # Handle Tiered Cashback Cards (like SAIB)
        if card.tiers:
            tier_vars = LpVariable.dicts(f"TierChoice_{card.name}", (t.name for t in card.tiers), cat=LpBinary)
            ...
            for tier in card.tiers:
                ...
                for cat in categories.values():
                    tier_cat_details = tier.categories.get(cat, TierCategory(rate=tier.base_rate, cap=float('inf')))
                    cashback_for_cat_in_tier = spend_vars[card.name, cat.key] * tier_cat_details.rate
                    activated_cat_cashback = LpVariable(f"Cashback_{card.name}_{tier.name}_{cat.key}", lowBound=0)
                    
                    prob += activated_cat_cashback <= M * y
                    prob += activated_cat_cashback <= cashback_for_cat_in_tier
                    if tier_cat_details.cap != float('inf'):
                         prob += cashback_for_cat_in_tier <= tier_cat_details.cap + M * (1-y)
        ...
        # Handle Cards with a Minimum Spend Requirement (like NBD)
            ...
            prob += total_spend_on_card >= card.min_spend_for_cashback - M * (1 - cashback_active)
            prob += total_spend_on_card <= (card.min_spend_for_cashback - 0.01) + M * cashback_active

            card_cashback = lpSum(
                spend_vars[card.name, cat.key] * card.categories.get(cat, CardCategory(rate=card.base_rate)).rate
                for cat in categories.values()
            )
        ...
        # Handle Regular and Lifestyle Cards
            ...
            base_cashback = lpSum(spend_vars[card.name, cat.key] * card.base_rate for cat in categories.values())
    ...
    prob += (total_monthly_cashback * 12) - total_annual_fees, "Total Net Annual Savings"

    # --- Constraints ---
    for cat in categories.values():
        prob += (lpSum(spend_vars[c.name, cat.key] for c in cards) == monthly_spending.get(cat.key, 0)), f"Spend_Total_{cat.key}"

    for card in cards:
        if card.tiers: continue
        
        if not isinstance(card, LifestyleCard) and card.monthly_cap != float("inf"):
            reg_card_cashback = lpSum(spend_vars[card.name, cat.key] * card.categories.get(cat, CardCategory(rate=card.base_rate)).rate for cat in categories.values())
            prob += reg_card_cashback <= card.monthly_cap, f"MonthlyCap_{card.name}"

        for cat, card_cat in card.categories.items():
            if card_cat.cap != float("inf"):
                prob += (spend_vars[card.name, cat.key] * card_cat.rate) <= card_cat.cap, f"CatCap_{card.name}_{cat.key}"

        if card.grouped_monthly_caps:
            for i, (cap, cat_list) in enumerate(card.grouped_monthly_caps):
                prob += (lpSum(spend_vars[card.name, c.key] * card.categories.get(c, CardCategory(rate=card.base_rate)).rate for c in cat_list) <= cap), f"GroupCap_{card.name}_{i}"
    ...
    if lifestyle_card:
        ...
        for plan in lifestyle_card.plans:
            potential_bonus = lpSum(
                spend_vars[lifestyle_card.name, category.key] * (card_category.rate - lifestyle_card.base_rate)
            ...
            prob += activated_bonus_vars[plan.name] >= potential_bonus - M * (1 - plan_vars[plan.name])

            for i, category_group in enumerate(plan.categories_rate_cap):
                cashback_in_group = lpSum(spend_vars[lifestyle_card.name, cat.key] * card_cat.rate for cat, card_cat in category_group.items())
                cap_value = list(category_group.values())[0].cap
                if cap_value != float("inf"):
                    prob += (cashback_in_group <= cap_value + M * (1 - plan_vars[plan.name])), f"PlanCap_{plan.name}_{i}"
...
def _process_optimization_results(
    ...
    results = [
        {"Card": c.name, "Category": cat.key, "Amount": spend_vars[c.name, cat.key].varValue}
        for c in cards
        for cat in categories.values()
